...
def train_epoch(args, device, model, criterion, optimizer, scheduler, train_loader, epoch, max_norm=1.0, **kwargs):
    ...
    for batch_idx, (image, label) in enumerate(train_loader):
        ...
        # ii = image[0].permute(1,2,0).cpu().detach().numpy()
        # ll = label[0].permute(1,2,0).cpu().detach().numpy()
    ...
    for batch_idx, (image, label) in enumerate(test_loader):
        with torch.no_grad():
            ...
            if (batch_idx + 1) % 10 == 0 or (batch_idx + 1) == len(test_loader):
                ...
                plot_test_results(image, resize(label), label_mask, epoch, batch_idx + 1, [0.0,0.0,0.0,0.0], save_root_path=args.save_root)
