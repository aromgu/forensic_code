...
from datasets.Dataset import ilove_Dataset, DSO_Dataset
...
def load_casia2(data_path, split_ratio, batch_size, img_size, num_workers) :
...
# AuTp
    train_dataset = ilove_Dataset(train_x, train_y, img_size, train_transform)
    test_dataset = ilove_Dataset(test_x, test_y, img_size, test_transform)
...
def load_casia1(data_path, split_ratio, batch_size, img_size, num_workers) :
    # Tp_image_path_CM = os.path.join(data_path, 'casia1groundtruth/CASIA1.0dataset/ModifiedTp/CM')
    Tp_image_path_Sp = os.path.join(data_path, 'casia1groundtruth/CASIA1.0dataset/ModifiedTp/Sp')
    # Tp_label_path_CM = os.path.join(data_path, 'casia1groundtruth/CASIA1.0groundtruth/CM')
    Tp_label_path_Sp = os.path.join(data_path, 'casia1groundtruth/CASIA1.0groundtruth/Sp')
    Au_image_path = os.path.join(data_path, 'casia1groundtruth/CASIA1.0dataset/Au')

    # Tp_images_CM = sorted([Tp_image_path_CM + '/' + x for x in os.listdir(Tp_image_path_CM)]) #[:10]
    Tp_images_Sp = sorted([Tp_image_path_Sp + '/' + x for x in os.listdir(Tp_image_path_Sp)]) #[:10]
    # Tp_labels_CM = sorted([Tp_label_path_CM + '/' + x for x in os.listdir(Tp_label_path_CM)]) #[:10]
    Tp_labels_Sp = sorted([Tp_label_path_Sp + '/' + x for x in os.listdir(Tp_label_path_Sp)]) #[:10]
    Au_images = sorted([Au_image_path + '/' + x for x in os.listdir(Au_image_path)]) #[:10]
    Au_labels = ['Au' for _ in range(len(Au_images))]
    Total_images = Tp_images_Sp + Au_images
    Total_labels = Tp_labels_Sp + Au_labels
...
def load_coverage(data_path, split_ratio, batch_size, img_size, num_workers) :
    Tp_image_path = os.path.join(data_path, 'COVERAGE/Tp')
    Tp_label_path = os.path.join(data_path, 'COVERAGE/Tp_mask')
    Au_image_path = os.path.join(data_path, 'COVERAGE/Au')
...
def load_dso1(data_path, split_ratio, batch_size, img_size, num_workers) :
    Tp_image_path = os.path.join(data_path, 'DSO-1/DSO1_DATA/DSO-1_SP')
    Tp_label_path = os.path.join(data_path, 'DSO-1/DSO1_DATA/DSO-1_MASK')
    Au_image_path = os.path.join(data_path, 'DSO-1/DSO1_DATA/DSO-1_AU')
...
# AuTp
    train_dataset = DSO_Dataset(train_x, train_y, img_size, train_transform)
    test_dataset = DSO_Dataset(test_x, test_y, img_size, test_transform)
