...
def plot_test_results(image, label, pred,  epoch, batch_idx, metrices, save_root_path) :
    label = label.squeeze().detach().cpu().numpy()
    pred = pred.squeeze().detach().cpu().numpy()
    pred_ = (pred >= 0.5).astype(np.int_)
    ...
    ax[3].imshow(pred_, cmap='gray')
    ax[3].axis('off'); ax[3].set_xticks([]); ax[3].set_yticks([]) #ax[2].set_title('HighPred')

    plt.suptitle("F1-Score : {}%  IoU : {}% Precision : {}% Recall : {}%".format(
        np.round(metrices[0] * 100, 4), np.round(metrices[1] * 100, 4), np.round(metrices[2] * 100, 4), np.round(metrices[3] * 100, 4)), fontsize=10)
