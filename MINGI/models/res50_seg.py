...
class Res(nn.Module):
    def __init__(self):
        ...
        modules = list(models.resnet50(pretrained=True).children())[:-3]
        # delete maxpooling
        del modules[3]
...
class Res50_seg(nn.Module):
    dim = [512, 256, 128, 64]
    def __init__(self,):
        super(Res50_seg, self).__init__()
        self.enc1 = Res()
        ...
        self.seam1 = SEAM(Res50_seg.dim[0]*3, Res50_seg.dim[0])

        # self.dec1 = ASPPBlock(Res50_seg.dim[0]*2 + 21, Res50_seg.dim[1])
        # self.dec2 = ASPPBlock(Res50_seg.dim[1]*2 + 21, Res50_seg.dim[2])
        # self.dec3 = ASPPBlock(Res50_seg.dim[2]*2 + 21, Res50_seg.dim[3])
        self.dec1 = nn.Sequential(
            nn.Conv2d(Res50_seg.dim[0]*2 + 21, Res50_seg.dim[1], kernel_size=(3,3), stride=(1,1), padding=(1,1)),
            nn.Upsample(scale_factor=2), nn.ReLU())
        self.dec2 = nn.Sequential(
            nn.Conv2d(Res50_seg.dim[1]*2 + 21, Res50_seg.dim[2], kernel_size=(3,3), stride=(1,1), padding=(1,1)),
            nn.Upsample(scale_factor=2), nn.ReLU())
        self.dec3 = nn.Sequential(
            nn.Conv2d(Res50_seg.dim[2]*2 + 21, Res50_seg.dim[3], kernel_size=(3,3), stride=(1,1), padding=(1,1)),
            nn.Upsample(scale_factor=2), nn.ReLU())
        self.rfam1 = RFAM(512)
        ...
        self.out = nn.Conv2d(Res50_seg.dim[3], 1, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))

    def forward(self, rgb, low, high):
        rgb_ = self.enc1.feature_extractor[0](rgb) # [b, 64, 128, 128]
        ...
        low1 = self.enc1.feature_extractor[3](low_)
        low2 = self.enc1.feature_extractor[4](low1)
        low3 = self.enc1.feature_extractor[5](low2)

        high_ = self.enc1.feature_extractor[0](high)
        high_ = self.enc1.feature_extractor[1](high_)
        high_ = self.enc1.feature_extractor[2](high_) # [b, 64, 128, 128]
