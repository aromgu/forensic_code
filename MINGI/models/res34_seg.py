...
class Res(nn.Module):
    def __init__(self):
        ...
        modules = list(models.resnet34(pretrained=True).children())[:-3]
        # delete maxpooling
        del modules[3]
...
class Res34_seg(nn.Module):
    dim = [256, 128, 64, 32]
    def __init__(self):
        super(Res34_seg, self).__init__()
        # ============
        self.enc1 = Res()
        self.rcu1 = RCU(64, 64)
        ...
        self.seam1 = SEAM(Res34_seg.dim[0]*3, Res34_seg.dim[0])

        # self.dec1 = ASPPBlock(Res34_seg.dim[0]*2 + 21, Res34_seg.dim[1])
        # self.dec2 = ASPPBlock(Res34_seg.dim[1]*2 + 21, Res34_seg.dim[2])
        # self.dec3 = ASPPBlock(Res34_seg.dim[2]*2 + 21, Res34_seg.dim[3])

        self.dec1 = nn.Sequential(
            nn.Conv2d(Res34_seg.dim[0]*2 + 21, Res34_seg.dim[1], kernel_size=(3,3), stride=(1,1), padding=(1,1)),
            nn.Upsample(scale_factor=2), nn.ReLU())
        self.dec2 = nn.Sequential(
            nn.Conv2d(Res34_seg.dim[1]*2 + 21, Res34_seg.dim[2], kernel_size=(3,3), stride=(1,1), padding=(1,1)),
            nn.Upsample(scale_factor=2), nn.ReLU())
        self.dec3 = nn.Sequential(
            nn.Conv2d(Res34_seg.dim[2]*2 + 21, Res34_seg.dim[3], kernel_size=(3,3), stride=(1,1), padding=(1,1)),
            nn.Upsample(scale_factor=2), nn.ReLU())
        ...
        self.out = nn.Conv2d(Res34_seg.dim[3], 1, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
    ...
    def forward(self, rgb, low, high):
        ...
        # Decoder # 더 할 꺼 있엉?
        dec1 = self.dec1(torch.cat([fuse, rgb_rcu3, scale1], dim=1)) # [b, 128, 64, 64]
        dec2 = self.dec2(torch.cat([dec1, rgb_rcu2, scale2], dim=1)) # [b, 64, 128, 128]
        dec3 = self.dec3(torch.cat([dec2, rgb_rcu1, scale3], dim=1))# + low_level) # [b, 32, 256, 256] # fuse, high add fuse
