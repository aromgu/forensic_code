...
class DUAL(nn.Module):
    ...
    def forward(self, rgb, low, high):
        ...
        bridge_wav_h = self.bn1(bridge_wav_h)
        bridge_wav_l = self.bn1(bridge_wav_l)
        ...
        up1 = self.aspp1(bridge_rgb * (((self.bam1(bridge_low+bridge_wav_l)) + (self.bam1(bridge_high+bridge_wav_h))))) # up  # [b, 1024, 32, 32]
        ...
        wav_128h = self.bn2(wav_128h)
        wav_128l = self.bn2(wav_128l)
        low3 = self.bn2(low3)
        ...
        cat2 = torch.cat([up2, rgb3 * ((self.bam2(low3+wav_128l)) + self.bam2(high3+wav_128h))], dim=1)  # 128
        ...
        cat3 = torch.cat([up3, rgb1 * (self.bam3(torch.add(low1, high1)))], dim=1) # 256
