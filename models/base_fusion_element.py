...
class Elementfusion(nn.Module):
    ...
    def forward(self, rgb, low, high):
        ...
        print(rgb1.shape, rgb2.shape, rgb3.shape, rgb4.shape)
        ...
        # DECODER
        up1 = self.aspp1(bridge_rgb * (torch.add(bridge_low, bridge_high))) # up
        conv1 = self.conv1(up1)

        up2 = self.upscale(conv1) # up
        cat2 = torch.cat([up2, rgb3 * (torch.add(low3, high3))], dim=1)
        conv2 = self.conv2(cat2)

        up3 = self.upscale(conv2) # up
        cat3 = torch.cat([up3, rgb1 * (torch.add(low1, high1))], dim=1)
...
if __name__ == '__main__':
    model = Elementfusion(2, 'resnet50', False, False, multi_grid=True, multi_dilation=[4, 8, 16]).cuda()
