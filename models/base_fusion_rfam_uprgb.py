...
class RFAMUPRGB(nn.Module):
    ...
    def forward(self, rgb, low, high):
        ...
        low1 = self.onebyone1(low1)
        low2 = self.onebyone2(low2)
        low3 = self.onebyone3(low3)
        low4 = self.onebyone4(low4)
        low1 = self.upscale2(low1) # 256
        low2 = self.upscale2(low2) # 128
        low3 = self.upscale1(low3) # 64

        low4 = self.down_spl(low4)
        bridge_low = self.bridge(low4) # 512
        bridge_low = self.aspp1(bridge_low)
        ...
        high1 = self.onebyone1(high1)
        high2 = self.onebyone2(high2)
        high3 = self.onebyone3(high3)
        high4 = self.onebyone4(high4)
        high1 = self.upscale2(high1) # 64
        high2 = self.upscale2(high2) # 128
        high3 = self.upscale1(high3) # 256

        high4 = self.down_spl(high4)
        bridge_high = self.bridge(high4) # 512
        bridge_high = self.aspp1(bridge_high)


        bridge_low = self.bn2(bridge_low)
        bridge_high = self.bn2(bridge_high)
        bridge_rgb = self.bn2(bridge_rgb)

        # DECODER
        up4 = self.bam1(bridge_low + self.bam1(bridge_high) + self.bam1(bridge_rgb)) # up 512

        up4 = self.bn2(up4)
        rgb4 = self.bn2(bridge_rgb)

        low4 = self.bn2(bridge_low)
        high4 = self.bn2(bridge_high)
        cat4 = torch.cat([up4, self.rfam4(low4, high4, rgb4)], dim=1) # 1024, 32, 32
...
####################
        ...
        cat3 = torch.cat([up3, self.rfam3(low3, high3, rgb3)], dim=1) # 512, 64, 64
...
####################
        ...
        cat2 = torch.cat([up2, self.rfam2(low2, high2, rgb2)], dim=1)  # 256
...
####################
        ...
        low1 = self.bn5(low1)
        high1 = self.bn5(high1)

        cat1 = torch.cat([up1, self.rfam1(low1, high1, rgb1)], dim=1)  # 128
