...
class Dfmbfi(nn.Module):
    ...
    def forward(self, rgb, low, high):
        ...
        print('conv1',self.pretrained.conv1)
        print('layer1',self.pretrained.layer1)
        print('layer2',self.pretrained.layer2)
        print('layer3',self.pretrained.layer3)
        print('layer4',self.pretrained.layer4)
        ...
        # DECODER
        bridge_high = self.bn1(bridge_high)
        bridge_low = self.bn1(bridge_low)
        bridge_rgb = self.bn1(bridge_rgb)

        up1 = self.aspp1(bridge_low + bridge_high + bridge_rgb) # up
        conv1 = self.conv1(up1)

        up2 = self.upscale(conv1) # up
        rgb3 = self.bn2(rgb3)
        low3 = self.bn2(low3)
        high3 = self.bn2(high3)
        # cat2 = torch.cat([up2, torch.add(torch.add(self.bam2(low3), self.bam2(high3)),rgb3)], dim=1)
        cat2 = torch.cat([up2, self.afimb1(low3, high3, rgb3)], dim=1)
        ...
        cat3 = torch.cat([up3, self.afimb2(low1, high1, rgb1)], dim=1)
...
if __name__ == '__main__':
    model = Dfmbfi(2, 'resnet50', False, False, multi_grid=True, multi_dilation=[4, 8, 16]).cuda()
